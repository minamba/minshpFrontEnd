...
export const CustomerAdmin = () => {
  ...
  }, [dispatch, page, pageSize, serverSearch, roleFilter]);
  ...
  const onAddRole = async () => {
    ...
    if (!aspId) { alert("IdAspNetUser manquant."); return; }
  ...
  const onChangeRole = async () => {
    ...
    if (!current) { await onAddRole(); return; }
    if (current === selectedRoleName) { closeRoleModal(); return; }
        ...
        <table className="table table-striped table-hover shadow-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={11} className="text-center">Aucun client.</td></tr>
            ) : (
              rows.map((c) => (
                <tr key={getId(c)}>
                  ...
                  <td>
                    <div className="d-flex gap-2">
                      ...
                      <button className="btn btn-sm btn-secondary" onClick={() => openRoleModal(c)} title="Droits / Rôles">
      ...
      {totalPages > 1 && (
        <div className="d-flex align-items-center justify-content-between mt-2">
          ...
          <nav className="d-flex align-items-center gap-2">
            ...
            <span>{startIdx}–{endIdx} sur {totalCount}</span>
            <button
              className="btn btn-light"
            ...
            <form onSubmit={handleSubmit}>
              <div className="row g-3">
                <div className="col-sm-4">
                  ...
                  <select className="form-select" name="civility" value={formData.civility} onChange={onFormChange}>
                ...
                <div className="col-sm-4">
                  ...
                  <input className="form-control" name="lastName" value={formData.lastName} onChange={onFormChange} required />
                </div>

                <div className="col-sm-4">
                  <label className="form-label">Prénom</label>
                  <input className="form-control" name="firstName" value={formData.firstName} onChange={onFormChange} required />
                </div>

                <div className="col-sm-6">
                  <label className="form-label">N° (téléphone)</label>
                  <input className="form-control" name="phone" value={formData.phone} onChange={onFormChange} />
                </div>

                <div className="col-sm-6">
                  <label className="form-label">Email</label>
                  <input type="email" className="form-control" name="email" value={formData.email} onChange={onFormChange} required />
                </div>

                <div className="col-sm-6">
                  <label className="form-label">Date de naissance</label>
                  <input type="date" className="form-control" name="birthDate" value={formData.birthDate} onChange={onFormChange} />
                </div>

                <div className="col-sm-6">
                  <label className="form-label">Pseudo</label>
                  <input className="form-control" name="pseudo" value={formData.pseudo} onChange={onFormChange} />
                </div>

                <div className="col-sm-6 d-flex align-items-center">
                  <div className="form-check mt-4">
                    <input className="form-check-input" type="checkbox" id="cust-active" name="actif" checked={!!formData.actif} onChange={onFormChange} />
        ...
        <div className="admin-modal-backdrop" role="presentation" onClick={closeRoleModal}>
          ...
          >
            ...
            <div className="d-flex justify-content-end gap-2">
              ...
              {canRemove(roleUser) ? (
                <button className="btn btn-primary" onClick={onChangeRole} disabled={!selectedRoleName || selectedRoleName === primaryRoleOf(roleUser)}>
                  Modifier le rôle
                </button>
              ) : (
